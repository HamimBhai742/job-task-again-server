...
const uri =
  "mongodb+srv://job_task_again:ScFC0UHSyRLHKDOc@cluster0.bls3tyg.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0";
  ...
  try {
    ...
    app.patch("/update-my-product/:id", async (req, res) => {
      ...
      const updateDoc = {
        $set: {
          ...
          productRating: parseFloat(products?.productRating),
          productDescription: products?.productDescription,
          offerPrice: parseFloat(products?.offerPrice),
        },
      };
      //   console.log(updateDoc);
    ...
    app.get("/productsPage", async (req, res) => {
      ...
      if (all?.type == "true") {
        ...
        } else if (all.sorting === "desc") {
          options.sort = {
            productPrice: -1,
          };
        } else if (all.sorting === "recently") {
          options.sort = {
            productDaTa: -1,
          };
        }
      ...
      const result = await productCollection
        ...
        .limit(size)
        .toArray();
      const result2 = await productCollection.find(querys, options).toArray();
